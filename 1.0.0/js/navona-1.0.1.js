...
$(document).ready(function () {
	$('.navona ul li').click(function () {
		if ($('#' + $(this).closest('div').attr('id')).data('navona-click')) {
			...
			$('#' + $(this).closest('div').attr('id') + ' ul li').width(((($(window).width() * 0.95) * closedSize)) / ($('#' + $(this).closest('div').attr('id') + ' ul li').length - 1));
			$(this).width(($(window).width() * 0.95) * openedSize);
		}
	});
...
function navonaSetSize(id) {
	var totalIamges = $('#' + id + ' ul li').length;
	...
	$('#' + id + ' ul li').width(((($(window).width() * 0.95) * closedSize)) / ($('#' + id + ' ul li').length - 1));
	$('#' + id + ' ul li:first-child').width(($(window).width() * 0.95) * openedSize);
	$('#' + id + ' ul li').children('img').each(function () {
		if (this.width > (($(window).width() * 0.95) * openedSize)) {
			$(this).width('100%');
			$(this).height('100%');
		} else {
			$(this).css('padding-top', ((($(window).height() * 0.91) / 2.5) - ((this.height) / 2)));
			$(this).css('padding-left', (((($(window).width() * 0.95) * openedSize) / 2) - ((this.width) / 2)));
		}
	});
